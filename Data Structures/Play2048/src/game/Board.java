...
public class Board {
    ...
    public void updateOpenSpaces() {
        ...
        // Iterate through every element of the gameBoard 2D array
        for(int i = 0; i < gameBoard.length; i++){
            for(int j = 0; j < gameBoard[i].length; j++)
            {
                ...
                if(gameBoard[i][j] == 0)
                {
                    // If it is, create a new BoardSpot object with the current indices i and j
                    BoardSpot openSpace = new BoardSpot(i, j);
                    // And add it to the openSpaces list
    ...
    public void addRandomTile() {
        ...
        int randomOpenIndex = StdRandom.uniform(0, openSpaces.size()); //generates a random index from the openSpaces array list
        BoardSpot randomOpenBoardSpot = openSpaces.get(randomOpenIndex); //uses the same index we just generated to get an actual open spot on the board

        double x = StdRandom.uniform(0.0, 1.0);  //genderates a random double between 0 and 1 in order to later figure out if we should assign our spot on the board a 2 or a 4.
        int tileValue;  //this is what we will call our spot on the board.
        if  (x < 0.1) 
        { 
            tileValue = 4;
        }
        ...
        gameBoard[randomOpenBoardSpot.getRow()][randomOpenBoardSpot.getCol()] =  tileValue;  // Update the tile in the gameBoard array with the proper value
    ...
    public void swipeLeft() 
    {
        ...
        //initialize a loop that goes row by row
        for (int row = 0; row < gameBoard.length; row++) 
        {
            int nextOpen = 0; //initialize a variable similar to the column variable in order to compare the two
            for (int col = 0; col < gameBoard[row].length; col++) 
            { //column variable initialized 
                if (gameBoard[row][col] != 0) //when the contents of a spot is not empty (if it is, the col variable will iterate to the next one)
                { 
                    if (nextOpen != col) //compare to see if it matches with our nextOpen variable or not 
                    { 
                        gameBoard[row][nextOpen] = gameBoard[row][col]; //if not, the contents of the col ptr will paste into the contents of the nextOpen ptr
                        gameBoard[row][col] = 0; //make the contents of the col ptr now empty
                    }
                    nextOpen++; //iterate nextOpen to the next column
                }
            }
    ...
    public void mergeLeft() 
    {
        ...
        for (int row = 0; row<gameBoard.length; row++) //initializes a for loop through the rows of the grid
        {
            for (int col = 0; col<gameBoard[row].length-1; col++) //initializes a loop through the columns of the rows of the grid
            {
                if (gameBoard[row][col] != 0 && gameBoard[row][col] == gameBoard[row][col+1]) //if the index is not empty and is equal to the the index of the next column 
                {
                    gameBoard[row][col] = gameBoard[row][col]*2; //multiply the left index by 2
                    gameBoard[row][col + 1] = 0; //set the right index to 0
                }
            }
        }
    ...
    public void transpose() {
        ...
        for (int row = 0; row < gameBoard.length; row++) //initializes a for loop through the rows of the grid
        { 
            for (int col = row; col < gameBoard[row].length; col++) //initializes a loop through the columns of the rows of the grid
            {
                int temp = gameBoard[row][col]; //stores the values in the position of [row][col] in variable temp
                gameBoard[row][col] = gameBoard[col][row]; //flips columns and rows
                gameBoard[col][row] = temp; //recalls the value we had stored in variable temp and replaces it
            }
        }
    ...
    public void flipRows() 
    {
       ...
       for (int row = 0; row < gameBoard.length; row++) //initializes a for loop through the rows of the grid
       {
        int ptrStart = 0; //initialize pointer 1 at the beginning
        int ptrEnd = gameBoard [row].length - 1; //initialize pointer 2 at the end of the row
            while (ptrStart < ptrEnd)  //while pointer 1 is behind pointer 2
            {
                int temp = gameBoard [row][ptrStart]; //initializes a temp variable in which we are storing the very first index of the first row
                gameBoard [row][ptrStart] = gameBoard [row][ptrEnd]; //sets the last index to the first index
                gameBoard [row][ptrEnd] = temp; //resets the last index to the first index
                ptrStart++; //increment
                ptrEnd--; //increment
            }
        } 
